...
<div class="container">
    <div class="row">
      ...
      <div class="col-md-8">
      <a href="#addgio" data-toggle="modal" class="btn btn-info">ایجاد سوالات جغرافیه</a>
        ...
        <!-- select question -->
           ...
           <table class="table table-bordered" id="example" >
            ...
            <tbody>
            @if(count($jiographic_question) > 0 )
                @foreach($jiographic_question as $jio)
              <tr>
                <td>{{$jio->id}}</td>
                <td>{{$jio->question}}</td>
                <td>{{$jio->option_a}}</td>
                ...
                <td>{{$jio->option_c}}</td>
                <td>{{$jio->option_d}}</td>
                <td>{{$jio->is_correct}}</td>
                <td>{{$jio->catagory[0]['catagory']}}</td>
                 <td><a href="#delete{{$jio->id}}" style="color:darkred; font-size:25px;" data-toggle="modal" class="glyphicon glyphicon-trash"></a>
                <a href="#update{{$jio->id}}" style="color:#1693c3; font-size:25px;" class="glyphicon glyphicon-edit" data-toggle="modal"></a>
                </td>
                </tr> 
                  <!-- delete question jio -->
  <div class="modal fade" id="delete{{$jio->id}}">
    <div class="modal-dialog">
      <div class="modal-content">
        ...
        <div class="modal-footer">
          <a href="delete_jio{{$jio->id}}" type="submit" id="submit"  class="btn btn-success">بلی</a>
        ...
        <!-- physic update  -->
    <div class="modal fade" id="update{{$jio->id}}">
    <div class="modal-dialog">
      <div class="modal-content">
        <div class="modal-header">
        <button type="button" aria-label="close" class="close" data-dismiss="modal">
          ...
          <form action="update_jio{{$jio->id}}" method="post">
            @csrf
           
            <div class="modal-body">
          <div class="form-group">
            <label for="">سوال</label>
            <input type="text" class="form-control" name="question" value="{{$jio->question}}" required>
          </div>
          <div class="form-group">
            <label for="">الف</label>
            <input type="text" class="form-control" name="option_a" value="{{$jio->option_a}}" required>
          </div>
          <div class="form-group">
            <label for="">ب</label>
            <input type="text" class="form-control" name="option_b" value="{{$jio->option_b}}" required>
          </div>
          <div class="form-group">
            <label for="">ج</label>
            <input type="text" class="form-control" name="option_c" value="{{$jio->option_c}}" required>
          </div>
          <div class="form-group">
            <label for="">د</label>
            <input type="text" class="form-control" name="option_d" value="{{$jio->option_d}}" required>
          </div>
          <div class="form-group">
            <label for="">جواب درست</label>
            <input type="text" class="form-control" name="is_correct" value="{{$jio->is_correct}}" required>
          </div>
              <div class="form-group">
...
<div class="modal fade" id="addgio">
    <div class="modal-dialog">
      <div class="modal-content">
        <div class="modal-header">
        <button type="button" aria-label="close" class="close" data-dismiss="modal">
          ...
          <form action="{{route('add_jio')}}" method="post">
            @csrf
